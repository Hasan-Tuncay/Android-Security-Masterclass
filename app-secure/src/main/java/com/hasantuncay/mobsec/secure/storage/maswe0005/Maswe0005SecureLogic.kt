...
object Maswe0005SecureLogic {
    ...
    fun executeVector(
        ...
        when (vector) {
            ...
            Maswe0005Mitigation.LOCAL_FILE -> secureLocalFileLeak(appData, context)
            Maswe0005Mitigation.SDK_TELEMETRY -> secureSdkTelemetryLeak(appData)
            Maswe0005Mitigation.WEBVIEW_CONSOLE -> secureWebViewConsoleLeak(appData, context)
        }

        if (vector == Maswe0005Mitigation.LOCAL_FILE) {
            onResult("files/diagnostics_secure.json")
        } else {
            val tag = when(vector) {
