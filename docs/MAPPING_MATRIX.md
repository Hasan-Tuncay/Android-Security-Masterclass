...
| **FileProvider Root-Path** | MASWE-0002 | MASVS-PLATFORM-2 | MASTG-TEST-0027 | CWE-284 | `<root-path path="/" />` exposes internal dirs | Scoped `<files-path>`, Strict Intent validation |
...
| **ContentProvider Path Traversal** | MASWE-0002 | MASVS-PLATFORM-2 | MASTG-TEST-0027 | CWE-22 | Trusting `?file=../../` query parameters | Validating `File.canonicalPath` boundaries |
| **Third-Party SDK Shadow Leak** | MASWE-0002 | MASVS-STORAGE-2 | MASTG-TEST-0002 | CWE-200 | 3rd party SDKs saving unhashed PII to SQLite offline | Hashing/Masking PII before sending it to analytics |
