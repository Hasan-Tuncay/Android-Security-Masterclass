...
This project follows the [Contributor Covenant Code of Conduct](https://www.contributor-covenant.org/version/2/1/code_of_conduct/code_of_conduct.md). By contributing, you agree to its terms. Be respectful, constructive, and security-minded in all communications.
...
### Step 1 — Claim an Issue First

Before writing any code, navigate to the GitHub Issues panel and claim an open `good first issue` or `help wanted` task that corresponds to a MASWE vector. Wait for a maintainer to assign the issue to you. **Do not implement unlisted vectors without prior discussion.**
...
* **Do not introduce memory leaks or unmanaged thread scopes (e.g., `thread {}` or unmanaged `CoroutineScope`). Use standard Coroutine practices even in vulnerable code unless the vulnerability specifically targets thread/memory mismanagement.**
...
### Step 5 — Implement the Attacker Logic (If Applicable)

If the vulnerability involves IPC, Intent Spoofing, FileProvider leaks, or Broadcast Receivers, you **must** implement a corresponding PoC exploit in the `:app-attacker` module to demonstrate the active exploitation of `:app-vulnerable`.

### Step 6 — Update `proguard-rules.pro` (if applicable)
...
**`docs/maswe/MASWE-[ID]-[Short-Name].md`** — Must contain:
...
* **A Cross-Reference mapping to specific MASTG test cases.**

**`docs/mastg-best/MASTG-BEST-XXXX-[Short-Name].md`** (if applicable) — For any corresponding MASTG best practice.

See [`docs/maswe/MASWE-0001-Logging-Leaks.md`](https://www.google.com/search?q=docs/maswe/MASWE-0001-Logging-Leaks.md) as the reference template.
...
* **Mutable sensitive data** (passwords, keys) must use `CharArray` or `ByteArray`, not `String`, and must be zeroed out when no longer needed.
...
3. **Never send raw PII to third-party SDKs.** Hash (with salt/KDF) or minimize first.
...
* [ ] `:app-attacker` PoC is implemented if the vector involves IPC/FileProvider.
* [ ] All code has inline KDoc explaining the security rationale.
* [ ] All standards referenced (MASWE, MASVS, MASTG, Android Docs) are cited correctly and mapped to test cases.
...
* [ ] CI pipeline checks (`detekt`, `lint`) pass locally before pushing.
...
5. A maintainer will review and may request changes. Automated CI checks will also run. Address all review comments and CI failures before re-requesting review.
...
| Missing Attacker PoC | If applicable, the lack of an exploit module leaves the theory unproven |
