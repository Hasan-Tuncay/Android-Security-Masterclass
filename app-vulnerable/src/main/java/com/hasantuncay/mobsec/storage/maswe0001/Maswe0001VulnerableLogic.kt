...
object Maswe0001VulnerableLogic {
    ...
    suspend fun executeVector(
        ...
        when (vector) {
            Maswe0001Vector.SHARED_PREFS_PLAINTEXT   -> triggerSharedPrefsLeak(appData, context, onResult)
            Maswe0001Vector.DATASTORE_UNENCRYPTED    -> triggerDataStoreLeak(appData, context, onResult)
            Maswe0001Vector.SQLITE_PLAINTEXT         -> triggerSqliteLeak(appData, context, onResult)
            Maswe0001Vector.FILE_PROVIDER_ROOT_PATH  -> triggerFileProviderLeak(appData, context, onResult)
            Maswe0001Vector.WEBVIEW_DOM_STORAGE      -> triggerWebViewLeak(appData, context, onResult)
            Maswe0001Vector.CACHE_DIRECTORY          -> triggerCacheLeak(appData, context, onResult)
            Maswe0001Vector.PATH_TRAVERSAL           -> triggerPathTraversalLeak(appData, context, onResult)
            Maswe0001Vector.THIRD_PARTY_SDK_LEAK     -> triggerSdkLeak(appData, context, onResult)
        }
    }
