...
class Maswe0007VulnerableViewModelTest {

    private val testDispatcher = StandardTestDispatcher()
    private val repository = mockk<Maswe0007VulnerableRepository>()
    ...
    private lateinit var viewModel: Maswe0007VulnerableViewModel

    @Before
    fun setUp() {
        Dispatchers.setMain(testDispatcher)
        every { masterclassDataRepository.masterclassData } returns testDataFlow
        viewModel = Maswe0007VulnerableViewModel(repository, masterclassDataRepository)
    }
    ...
    fun `initial state is Idle`() = runTest {
        assertEquals(Maswe0007VulnerableState(), viewModel.uiState.value)
    }

    @Test
    fun `execute vector updates state to Loading then Success`() = runTest {
        val vector = Maswe0007Vector.BROKEN_ALGORITHM
        val expectedResult = "EXPOSED (DES 56-bit): ciphertext"
        ...
        viewModel.uiState.test {
            ...
            viewModel.processIntent(Maswe0007VulnerableIntent.ExecuteVector(vector))
    ...
    fun `execute vector failure updates state to Error and emits effect`() = runTest {
        val vector = Maswe0007Vector.ECB_MODE
        val errorMessage = "ECB encryption simulation failed"
        ...
        viewModel.effect.test {
            ...
            assertTrue(effect is Maswe0007VulnerableEffect.ExecutionFailed)
            assertEquals(errorMessage, (effect as Maswe0007VulnerableEffect.ExecutionFailed).error.message)
        }
    ...
    fun `reset intent clears selected vector and sets state to Idle`() = runTest {
        val vector = Maswe0007Vector.REUSED_IV
        coEvery { repository.executeVector(vector, any()) } returns "result"

        viewModel.processIntent(Maswe0007VulnerableIntent.ExecuteVector(vector))
        testDispatcher.scheduler.advanceUntilIdle()

        viewModel.processIntent(Maswe0007VulnerableIntent.Reset)
        testDispatcher.scheduler.advanceUntilIdle()
