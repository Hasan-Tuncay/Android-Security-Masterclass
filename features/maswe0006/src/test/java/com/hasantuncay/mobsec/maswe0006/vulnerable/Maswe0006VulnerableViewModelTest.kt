...
class Maswe0006VulnerableViewModelTest {

    private val testDispatcher = StandardTestDispatcher()
    private val repository = mockk<Maswe0006VulnerableRepository>()
    ...
    private lateinit var viewModel: Maswe0006VulnerableViewModel

    @Before
    fun setUp() {
        Dispatchers.setMain(testDispatcher)
        every { masterclassDataRepository.masterclassData } returns testDataFlow
        viewModel = Maswe0006VulnerableViewModel(repository, masterclassDataRepository)
    }
    ...
    fun `initial state is Idle`() = runTest {
        assertEquals(Maswe0006VulnerableState(), viewModel.uiState.value)
    }

    @Test
    fun `execute vector updates state to Loading then Success`() = runTest {
        val vector = Maswe0006Vector.AUTOMATIC_SYSTEM_BACKUP
        val expectedResult = "EXPOSED IN AUTO-BACKUP: maswe0006_cloud_backup_session.xml"
        ...
        viewModel.uiState.test {
            ...
            viewModel.processIntent(Maswe0006VulnerableIntent.ExecuteVector(vector))
    ...
    fun `execute vector failure updates state to Error and emits effect`() = runTest {
        val vector = Maswe0006Vector.LOCAL_BACKUP
        val errorMessage = "Failed to write backup simulation file"
        ...
        viewModel.effect.test {
            ...
            assertTrue(effect is Maswe0006VulnerableEffect.ExecutionFailed)
            assertEquals(errorMessage, (effect as Maswe0006VulnerableEffect.ExecutionFailed).error.message)
        }
    ...
    fun `reset intent clears selected vector and sets state to Idle`() = runTest {
        val vector = Maswe0006Vector.DEVICE_TO_DEVICE_TRANSFER
        coEvery { repository.executeVector(vector, any()) } returns "result"

        viewModel.processIntent(Maswe0006VulnerableIntent.ExecuteVector(vector))
        testDispatcher.scheduler.advanceUntilIdle()

        viewModel.processIntent(Maswe0006VulnerableIntent.Reset)
        testDispatcher.scheduler.advanceUntilIdle()
