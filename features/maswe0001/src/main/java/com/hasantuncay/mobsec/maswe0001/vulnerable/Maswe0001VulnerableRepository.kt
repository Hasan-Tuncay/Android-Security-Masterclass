...
class Maswe0001VulnerableRepository @Inject constructor(
    ...
    // The DataStore file will be written to: /data/data/<pkg>/files/datastore/maswe0001_store.preferences_pb
    // ──────────────────────────────────────────────────────────────────────────
    private val Context.maswe0001DataStore by preferencesDataStore(name = "maswe0001_store")
    ...
    suspend fun executeVector(
        ...
        return when (vector) {
            Maswe0001Vector.SHARED_PREFS_PLAINTEXT   -> triggerSharedPrefsLeak(appData)
            Maswe0001Vector.DATASTORE_UNENCRYPTED    -> triggerDataStoreLeak(appData)
            Maswe0001Vector.SQLITE_PLAINTEXT         -> triggerSqliteLeak(appData)
            Maswe0001Vector.FILE_PROVIDER_ROOT_PATH  -> triggerFileProviderLeak(appData)
            Maswe0001Vector.WEBVIEW_DOM_STORAGE      -> triggerWebViewLeak(appData)
            Maswe0001Vector.CACHE_DIRECTORY          -> triggerCacheLeak(appData)
            Maswe0001Vector.PATH_TRAVERSAL           -> triggerPathTraversalLeak(appData)
            Maswe0001Vector.THIRD_PARTY_SDK_LEAK     -> triggerSdkLeak(appData)
        }
    }
     ...
     * adb shell run-as com.hasantuncay.mobsec cat shared_prefs/maswe0001_session.xml
     */
    private suspend fun triggerSharedPrefsLeak(
        appData: MasterclassData
    ): String? {
        var resultPath: String? = null
        val prefs = context.getSharedPreferences("maswe0001_session", Context.MODE_PRIVATE)
        prefs.edit().apply {
        ...
        val filePath = "${context.applicationInfo.dataDir}/shared_prefs/maswe0001_session.xml"
        ...
        Log.e(
            ...
            File: $filePath
            Verification: adb shell run-as com.hasantuncay.mobsec cat shared_prefs/maswe0001_session.xml
            """.trimIndent()
        )
     ...
     *
     * Why it fails:
     * 1. ROOT ATTACK: `adb shell run-as com.hasantuncay.mobsec strings files/datastore/maswe0001_store.preferences_pb`
     * 2. ADB BACKUP: Included in adb backup output by default.
     * 3. PROTOBUF DECODE: `protoc --decode_raw < maswe0001_store.preferences_pb`
     *
     * ADB Verification Command:
     * adb shell run-as com.hasantuncay.mobsec cat files/datastore/maswe0001_store.preferences_pb | strings
     */
    private suspend fun triggerDataStoreLeak(
        ...
        withContext(Dispatchers.IO) {
            appContext.maswe0001DataStore.edit { prefs ->
            ...
            val filePath = "${appContext.applicationInfo.dataDir}/files/datastore/maswe0001_store.preferences_pb"
            Log.e(
                "VULN_0002_DATASTORE",
                """
                ...
                Verification: adb shell run-as com.hasantuncay.mobsec cat files/datastore/maswe0001_store.preferences_pb | strings
                """.trimIndent()
            )
            withContext(Dispatchers.Main) {
                resultPath = filePath
    ...
    private suspend fun triggerSqliteLeak(
        ...
        withContext(Dispatchers.IO) {
            ...
            val dbPath   = "${context.applicationInfo.dataDir}/databases/maswe0001_vuln.db"
            withContext(Dispatchers.Main) {
                resultPath = dbPath
            }
        }
    ...
    private suspend fun triggerFileProviderLeak(
        ...
        var resultPath: String? = null
        withContext(Dispatchers.IO) {
            val prefs = context.getSharedPreferences("maswe0001_fileprovider", Context.MODE_PRIVATE)
            ...
            val file = File(context.applicationInfo.dataDir, "shared_prefs/maswe0001_fileprovider.xml")
            if (!file.exists()) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(context, context.getString(CommonR.string.error_file_not_created), Toast.LENGTH_SHORT).show()
                }
                return@withContext
            }
    ...
    private suspend fun triggerExternalStorageLeak(
        ...
        withContext(Dispatchers.IO) {
            ...
            if (externalDir == null) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(context, context.getString(CommonR.string.error_external_storage_not_mounted), Toast.LENGTH_SHORT).show()
                }
                return@withContext
            }

            val sensitiveFile = File(externalDir, "maswe0001_external_leak.json")
            ...
            withContext(Dispatchers.Main) {
                resultPath = sensitiveFile.absolutePath
            }
        }
        return resultPath
    }
    ...
    private suspend fun triggerWebViewLeak(
        ...
        withContext(Dispatchers.Main) {
            try {
                ...
                resultPath = "WebView Error: ${e.message}"
            }
        }
        return resultPath
    }
    ...
    private suspend fun triggerPathTraversalLeak(
        ...
        withContext(Dispatchers.IO) {
            ...
            val exploitUri = "content://${context.packageName}.vulnerable.provider/download?file=../shared_prefs/maswe0001_session.xml"
    ...
    private suspend fun triggerSdkLeak(
        ...
        withContext(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                resultPath = shadowDb.absolutePath
            }
        }
        return resultPath
    }
